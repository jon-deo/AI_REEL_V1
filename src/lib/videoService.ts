...
async function createVideoFromImagesAndAudio(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (imageFiles.length === 0) {
      reject(new Error("No image files provided for video creation"));
      return;
    }
    
    // Log the image files we're using
    console.log(`Creating video from ${imageFiles.length} images:`, imageFiles);
    ...
    // Add images as inputs with duration settings and optimize for speed
    imageFiles.forEach((img, index) => {
      ...
      console.log(`Adding image ${index} to video: ${img}`);
      command.input(img).inputOptions([`-loop 1`, `-t ${imageDuration}`]);
    });
    ...
    command
      .complexFilter([
        ...
        {
          filter: 'concat',
          options: { n: imageFiles.length, v: 1, a: 0 },
          inputs: imageFiles.map((_, i) => `${i}:v`),
          outputs: ['v']
        },
        // Add audio
        {
          filter: 'apad',
          inputs: [`${imageFiles.length}:a`],
          outputs: ['a']
        },
      ...
      // Optimization flags for faster encoding but maintain quality
      .outputOptions([
        '-map [v]', 
        '-map [amixed]', 
        '-shortest',
        ...
        '-tune film',          // Better for image sequences
        '-crf 23',             // Better quality (lower = better, 23 is good balance)
